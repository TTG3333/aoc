...
grid[startPos[1]][startPos[0]] = {"height": 0, "dist": 0}
grid[endPos[1]][endPos[0]] = {"height": 25, "dist": float("inf")}

unvisited = {(x, y) for x in range(len(grid[0])) for y in range(len(grid))}
current = tuple(startPos)
around = ((-1, 0), (0, 1), (1, 0), (0, -1))
while tuple(endPos) in unvisited: # Dijkstra
...
print(grid[endPos[1]][endPos[0]]["dist"])